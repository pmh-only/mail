<script lang="ts">
  ...
  let {
    ...
    compact = false,
    iconOnly = false
  }: { attachment: Attachment; compact?: boolean; iconOnly?: boolean } = $props()
...
{#if supported}
  <div class={iconOnly ? 'contents' : compact ? 'mt-2 w-full' : 'border-t border-white/8 px-2.5 py-2'}>
    <button
      type="button"
      onclick={summarize}
      disabled={loading}
    ...
    >
      <Sparkles size={iconOnly ? 14 : 12} class={loading ? 'animate-pulse' : ''} />
      {#if !iconOnly}
    ...
    {#if summary}
      <div
        class="mt-2 rounded-lg border border-white/8 bg-black/20 p-2 text-xs leading-relaxed whitespace-pre-wrap text-zinc-300 {iconOnly
          ? 'order-2 basis-full'
          : ''}"
      >
        {summary}
    ...
    {/if}
    {#if errorMessage}
      <p class="mt-2 text-xs text-red-300 {iconOnly ? 'order-2 basis-full' : ''}">
        {errorMessage}
      </p>
    {/if}
  </div>
